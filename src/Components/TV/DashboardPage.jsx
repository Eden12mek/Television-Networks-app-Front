...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    setChartOptions({
      colors: ["#1C64F2", "#16BDCA", "#FDBA8C", "#FB977DD9", "#16BDCA"],
      chart: {
        type: "donut",
        sparkline: {
          enabled: true,
  ...
  useEffect(() => {
    const getGraphOptions = () => {
      const daysOfWeek = ["Mon", "Tue", "Wed", "Thu", "Fri"]; // Define your day labels here

      const seriesData = Object.values(programsByType).map(type => ({
        name: type.name,
        data: type.programs.map((program, index) => ({
          x: index,
          y: program.count, // Assuming 'count' represents the y-axis value
        })),
      }));
    ...
    return () => {
      if (graphRef.current) {
        graphRef.current.innerHTML = ''; // Clear the chart container if unmounted
      }
    };
  }, [programsByType]);
              ...
              <Box display="flex" flexDirection={{ xs: 'column', md: 'row' }} gap={2} mt={7} bgcolor="white" borderRadius={2} boxShadow={3} p={3}>
                <Grid container spacing={2}>
                  <Grid item xs={12} md={7}>
                    ...
                    {/* <div ref={graphRef}></div> This div will contain the ApexCharts graph */}
                  </Grid>
                  <Grid item xs={12} md={5}>
                    ...
                    <Grid container spacing={3}>
                      {Object.values(programsByType).map(type => (
                        <Grid item xs={12} md={4} key={type.id} style={{ width: '400px' }}>
                          <Typography variant="h6" gutterBottom>
                            {type.name} ({type.count})
                          </Typography>
                          {type.programs.map((program, index) => (
                            <Box key={program.id} display="flex" alignItems="center" gap={0.5} mt={index > 0 ? 1 : 0}>
                              <Avatar sx={{ bgcolor: ['green', 'fuchsia', 'pink', 'green', 'sky'][index % 5], height: 30, width: 30 }} />
                              <Typography variant="body1">{program.name}</Typography>
                              <Typography variant="body2" ml="auto" color="text.secondary">{program.rating}</Typography>
                            </Box>
                          ))}
                        </Grid>
                      ))}
